...
export const checkVerificationStatus = async (args, projectDir) => {
    const raw = args[0];
    if (!raw) {
    ...
    if (existsSync(phaseDirFull)) {
        try {
            ...
            if (verFile)
                verPath = join(phaseDirFull, verFile);
        }
        catch {
            return { data: NOT_FOUND_RESULT };
        }
    ...
    const isHeader = firstRow.cells.some(c => /^(id|status|description|type|notes)$/i.test(c));
    ...
    for (const row of dataRows) {
        ...
        if (statusVal === 'PASS' || statusVal === 'FAIL')
            totalCount++;
        if (statusVal === 'PASS')
            passCount++;
        if (statusVal === 'FAIL') {
...
//# sourceMappingURL=check-verification-status.js.map