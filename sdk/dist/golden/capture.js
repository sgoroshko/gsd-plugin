    ...
    return new Promise((resolve, reject) => {
        execFile(process.execPath, fullArgs, {
            ...
            if (err) {
                ...
                reject(new Error(`gsd-tools failed (exit ${code}): ${stderrStr || (err instanceof Error ? err.message : String(err))}`));
                return;
            }
...
async function parseGsdToolsJson(raw, projectDir) {
 ...
 */
export async function captureGsdToolsOutput(command, args, projectDir) {
 ...
 */
export async function captureGsdToolsStdout(command, args, projectDir) {
    const { stdout } = await execGsdTools(projectDir, command, args);
    return stdout;
}
